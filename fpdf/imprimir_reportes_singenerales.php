...
@ini_set('memory_limit', '128M'); 
	...
	$pdf->nombrereporte = "Reporte Varios - Sin Generales";
	...
	$header_type2[5]['WIDTH'] = $funcionesgenerales->CPaN($tamano_hoja_mm,10);//Nombres
	$header_type2[6]['WIDTH'] = $funcionesgenerales->CPaN($tamano_hoja_mm,9);//Ap. Paterno
	$header_type2[7]['WIDTH'] = $funcionesgenerales->CPaN($tamano_hoja_mm,7);//Ap. Materno
	$header_type2[8]['WIDTH'] = $funcionesgenerales->CPaN($tamano_hoja_mm,5);//Fecha Nacimiento
	...
	$header_type2[9]['WIDTH'] = $funcionesgenerales->CPaN($tamano_hoja_mm,7);//Documentos
	$header_type2[10]['WIDTH'] = $funcionesgenerales->CPaN($tamano_hoja_mm,2);//no categoria
	$header_type2[11]['WIDTH'] = $funcionesgenerales->CPaN($tamano_hoja_mm,2);//Validado
	$header_type2[12]['WIDTH'] = $funcionesgenerales->CPaN($tamano_hoja_mm,5.5);//modalidad
	$header_type2[13]['WIDTH'] = $funcionesgenerales->CPaN($tamano_hoja_mm,5.5);//deporte	
	$header_type2[14]['WIDTH'] = $funcionesgenerales->CPaN($tamano_hoja_mm,11.5);//categoria
	$header_type2[15]['WIDTH'] = $funcionesgenerales->CPaN($tamano_hoja_mm,9.5);//prueba	total:98
	...
	$header_type2[10]['TEXT'] = "No.";	
	$header_type2[10]['T_ALIGN'] = "L";
	
	$header_type2[11]['TEXT'] = "Val.";	
	$header_type2[11]['T_ALIGN'] = "L";
	
	$header_type2[12]['TEXT'] = "Modalidad";	
	$header_type2[12]['T_ALIGN'] = "L";
	
	$header_type2[13]['TEXT'] = "Deporte";	
	$header_type2[13]['T_ALIGN'] = "L";
	
	$header_type2[14]['TEXT'] = "Categoria";	
	$header_type2[14]['T_ALIGN'] = "L";
	
	$header_type2[15]['TEXT'] = "Pruebas";
	$header_type2[15]['T_ALIGN'] = "L";
		...
		{			        
				...
				{				    
				 ...
				 // $pdf->ImagePngWithAlpha("../fotosparticipantes/".$curp.".png",0,20,11,16);
				  ...
				  $pdf->tbDrawData($data);			  
				  $arraycategorias = explode('{y}',$caracteristicas);				  
				  $nchar = 65;			  
				  
				  foreach ($arraycategorias as $val) {			  				      
					  $datodesglosearr = explode('{x}', $val);				  					 
					  $data = Array();				  
					 
					  $data[10]['TEXT'] = chr($nchar).'.';				  				  
					  ...
					  $nchar++;
					  foreach ($datodesglosearr as $datoarr) {	                   			  
						...
						if($dato[0]=='M'){  
						  $data[12]['TEXT'] = html_entity_decode($dato[1]);				  
						...
						if($dato[0]=='D'){
						  $data[13]['TEXT'] = html_entity_decode($dato[1]);				  
						  $data[13]['T_SIZE'] = $fsize;
						  $data[13]['T_ALIGN'] = "L";
						  $data[13]['V_ALIGN'] = "T";
						  $data[13]['LN_SIZE'] = $separacion_linea; 
						}
						if($dato[0]=='C'){
						  $data[14]['TEXT'] = html_entity_decode($dato[1]);				  
						  $data[14]['T_SIZE'] = $fsize;
						  $data[14]['T_ALIGN'] = "L";
						  $data[14]['V_ALIGN'] = "T";
						  $data[14]['LN_SIZE'] = $separacion_linea;
						}
						if($dato[0]=='P'){
						  $data[15]['TEXT'] = html_entity_decode($dato[1]);				  
						  $data[15]['T_SIZE'] = $fsize;
						  $data[15]['T_ALIGN'] = "L";
						  $data[15]['V_ALIGN'] = "T"; 
						  $data[15]['LN_SIZE'] = $separacion_linea;				   
						}
						if($dato[0]=='AI'){
						  $data[14]['TEXT'] = $data[14]['TEXT']." (". html_entity_decode($dato[1]);				  
						  $data[14]['T_SIZE'] = $fsize;
						  $data[14]['T_ALIGN'] = "L";
						  $data[14]['V_ALIGN'] = "T"; 
						  $data[14]['LN_SIZE'] = $separacion_linea;				   
