(function(jQuery) {
	...
	jQuery.fn.select_autocomplete = function(options) {
		...
		var defaults = { 
			  minChars: 0
			, width: jQuery('.ac_input').attr('width')
			, matchContains: true
			, autoFill: false
		...
		return this.each(function() {
			...
			$input.result(function(event, selected_item, formatted) { 
				jQuery($this.find('option[value=' + selected_item.value + ']')[0]).attr('selected', true);
			});
      ...
      $input.blur(function(){
          ...
          if(this.value == ""){
            jQuery($this.find('option[value=]')[0]).attr('selected', true);
          }
