...
function readdata()
{
	...
	jQuery.post("../scripts/sitd_script_lista_catmoded.php", 
			  {paginado: paginado, pagina: pagina, limite: limite, filtro: filtro, campo:  campo, orden:  orden, idusuario:  jQuery('#idusuario').val(), eventonacional:jQuery('#filter_eventonacional').val()}, function(data){loaddata(data);repeat_ready();}
	);
}
...
{	
    if(tipo=='addall'){		
	    if(jQuery('.check_me:checked').length>=1){
	       ...
	       location.href="sitd_mod_editargrupo_catmoded.php?ids="+array_check_me+"&tipo="+tipo+"&texto="+texto;			
	    }else{mensaje('Seleccione un o m&aacute;s elemento de la lista, para editar...','error','Error');}	
	}

    if(tipo=='edit'){		
		if(jQuery('.check_me:checked').length==1){
			location.href="sitd_mod_guardaraplicar_catmoded.php?id="+jQuery('.check_me:checked').val()+"&tipo="+tipo+"&texto="+texto;			
		}else{
	...
	if(tipo=='add'){
		location.href="sitd_mod_guardaraplicar_catmoded.php?id=0&tipo="+tipo+"&texto="+texto;		
	}
}
...
{	   
    ...
    jQuery(".check_me:checked").each(function(){array_check_me.push(this.value)});
	if(array_check_me.length>0){
	jQuery.post("../scripts/sitd_script_borrar_catmoded.php", 
