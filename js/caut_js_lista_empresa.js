...
function readdata()
{
	...
	jQuery.post("../scripts/caut_script_lista_empresa.php", 
...
{	
    if(tipo=='edit'){		
		if(jQuery('.check_me:checked').length==1){
			location.href="caut_mod_guardaraplicar_empresa.php?id="+jQuery('.check_me:checked').val()+"&tipo="+tipo+"&texto="+texto;			
		}else{
	...
	if(tipo=='add'){
		location.href="caut_mod_guardaraplicar_empresa.php?id=0&tipo="+tipo+"&texto="+texto;		
	}
}
...
{	
	...
	jQuery.post("../scripts/caut_script_status_empresa.php", 
...
function borrar()
{
    ...
    jQuery(".check_me:checked").each(function(){array_check_me.push(this.value)});
	if(array_check_me.length>0){
	jQuery.post("../scripts/caut_script_borrar_empresa.php", 
