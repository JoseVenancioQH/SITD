...
{  	          	 
    if(jQuery("#form_catevenac").validationEngine('validate')){guardar_aplicar(tipo);}else{ var sliderIntervalID = setInterval(function(){jQuery("#form_catevenac").validationEngine('hideAll'); clearInterval(sliderIntervalID);},5000);}
}
...
{	    
   ...
   jQuery.post("../scripts/sitd_script_"+tipo+"_catevenac.php", {nombreevenac: jQuery('#evenac').val() , fechainicio: jQuery('#fechainicio').val(), fechatermina: jQuery('#fechatermina').val(), idusuario:  jQuery('#idusuario').val(), id: jQuery('#id').val()}, function(data){loaddata(data,tipo);});
}

function loaddata(data,tipo)
{	
    mensaje(data,6000);
	if(tipo == 'guardartodo' || tipo == 'aplicar'){var sliderIntervalID = setInterval(function(){location.href="../modulos/sitd_mod_lista_catevenac.php"; clearInterval(sliderIntervalID);},7000);}else
	{jQuery('#evenac').val('');jQuery('#filter_eventonacional').val('');}    
}

function edit_data()
{ 
  if(jQuery('#id').val()!='0')
  jQuery.post("../scripts/sitd_script_editar_catevenac.php", {id: jQuery('#id').val()}, function(data){load_edit_data(data);},"json");  
}

function load_edit_data(data)
{	   
	jQuery.each(data, function(i,item){								   
	   jQuery('#evenac').val(item.nombreevenac);	   
