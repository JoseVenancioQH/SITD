...
function readdata()
{
	...
	jQuery.post("../scripts/sitd_script_lista_catasocdepmd.php", 
			  {paginado: paginado, pagina: pagina, limite: limite, filtro: filtro, campo:  campo, orden:  orden, idusuario:  jQuery('#idusuario').val(), asocdep:jQuery('#filter_asocdep').val()}, function(data){loaddata(data);repeat_ready();jQuery.unblockUI();}
	);
}
...
{	
    if(tipo=='addall'){	
		if(jQuery('.check_me:checked').length>=1){
		  ...
		  location.href="sitd_mod_editargrupo_catasocdepmd.php?ids="+array_check_me+"&tipo="+tipo+"&texto="+texto;			
		}else{
    ...
    if(tipo=='edit'){		
		if(jQuery('.check_me:checked').length==1){
			location.href="sitd_mod_guardaraplicar_catasocdepmd.php?id="+jQuery('.check_me:checked').val()+"&tipo="+tipo+"&texto="+texto;			
		}else{
			 mensaje('{"tipo":"error","mensaje":"Seleccione un elemento de la lista, para editar..."}',4000);			 			 
		}				
	}
	...
	if(tipo=='add'){
		location.href="sitd_mod_guardaraplicar_catasocdepmd.php?id=0&tipo="+tipo+"&texto="+texto;		
	}
}
...
{	
    ...
    jQuery(".check_me:checked").each(function(){array_check_me.push(this.value)});
	if(array_check_me.length>0){
	jQuery.post("../scripts/sitd_script_borrar_catasocdepmd.php", 
