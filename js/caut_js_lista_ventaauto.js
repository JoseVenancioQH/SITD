...
function readdata()
{
	...
	jQuery.post("../scripts/caut_script_lista_invauto.php", 
...
{	
    if(tipo=='edit'){		
		if(jQuery('.check_me:checked').length==1){
			location.href="caut_mod_guardaraplicar_ventaauto.php?id="+jQuery('.check_me:checked').val()+"&tipo="+tipo+"&texto="+texto;			
		}else{
	...
	if(tipo=='add'){
		alert('no entra add');
		location.href="caut_mod_guardaraplicar_ventaauto.php?id=0&tipo="+tipo+"&texto="+texto;		
	}
}
...
{	
    ...
    jQuery(".check_me:checked").each(function(){array_check_me.push(this.value)});
	if(array_check_me.length>0){
	jQuery.post("../scripts/caut_script_borrar_invauto.php", 
