...
function readdata()
{
	...
	jQuery.post("../scripts/sitd_script_lista_catrama.php", 
...
{	
    ...
    if(tipo=='edit'){		
		if(jQuery('.check_me:checked').length==1){
			location.href="sitd_mod_guardaraplicar_catrama.php?id="+jQuery('.check_me:checked').val()+"&tipo="+tipo+"&texto="+texto;			
		}else{
			 mensaje('{"tipo":"error","mensaje":"Seleccione un elemento de la lista, para editar..."}',4000);			 			 
		}				
	}
	...
	if(tipo=='add'){
		location.href="sitd_mod_guardaraplicar_catrama.php?id=0&tipo="+tipo+"&texto="+texto;		
	}
}
