...
function readdata()
{
	...
	jQuery.post("../scripts/sitd_script_lista_usuarios.php", 
...
{	
    if(tipo=='edit'){		
		if(jQuery('.check_me:checked').length==1){
			location.href="sitd_mod_guardaraplicar_usuarios.php?id="+jQuery('.check_me:checked').val()+"&tipo="+tipo+"&texto="+texto;			
		}else{
	...
	if(tipo=='add'){
		location.href="sitd_mod_guardaraplicar_usuarios.php?id=0&tipo="+tipo+"&texto="+texto;		
	}
}
...
{	
	...
	jQuery.post("../scripts/sitd_script_status_usuarios.php", 
...
function borrar()
{
    ...
    jQuery(".check_me:checked").each(function(){array_check_me.push(this.value)});
	if(array_check_me.length>0){
	jQuery.post("../scripts/sitd_script_borrar_usuarios.php", 
