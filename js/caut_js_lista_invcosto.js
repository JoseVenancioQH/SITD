...
function readdata()
{
	...
	jQuery.post("../scripts/caut_script_lista_invcosto.php", 
			  {paginado: paginado, pagina: pagina, limite: limite, filtro: filtro, campo:  campo, orden:  orden, idempresa:  jQuery('#idempresa').val(), marca:jQuery('#filter_marca').val(), modelo:jQuery('#filter_modelo').val(), linea:jQuery('#filter_linea').val(), tipo:jQuery('#filter_tipo').val(), color:jQuery('#filter_color').val()}, function(data){loaddata(data);repeat_ready();jQuery.unblockUI();}
	);
}
...
{	
    if(tipo=='edit'){		
		if(jQuery('.check_me:checked').length==1){
			location.href="caut_mod_guardaraplicar_invcosto.php?id="+jQuery('.check_me:checked').val()+"&tipo="+tipo+"&texto="+texto;			
		}else{
	...
	if(tipo=='add'){
		location.href="caut_mod_guardaraplicar_invcosto.php?id=0&tipo="+tipo+"&texto="+texto;		
	}
}
...
{	
	...
	jQuery.post("../scripts/caut_script_status_invcosto.php", 
...
{	
    ...
    jQuery(".check_me:checked").each(function(){array_check_me.push(this.value)});
	if(array_check_me.length>0){
	jQuery.post("../scripts/caut_script_borrar_invcosto.php", 
