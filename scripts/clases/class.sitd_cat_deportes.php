...
class catdeportes extends MySQL
{   
    var $pagina = '';
	var $paginado = '';
	var $limite = '';
	...
	function borrarCatDeportes()
	{		    
	    $array_ids = array();	
		
		foreach (explode(',',$this->ids) as $id_act){	
		         $consulta = parent::consulta("SELECT id_deportes FROM cat_deportes as r WHERE r.id_deportes = ".$id_act);
				 $num_total_registros = parent::num_rows($consulta);
				 if($num_total_registros!=0)
				  {	
					$consulta = parent::consulta("DELETE FROM cat_deportes where id_deportes = ".$id_act);
					if(mysql_affected_rows() > 0) $array_ids[]=$id_act;
				  }				
		}
	    return implode(',',$array_ids);
	...
	function grabarCatDeportes()
    {
	   $consulta = parent::consulta("SELECT cat_deportes_nombre FROM cat_deportes WHERE cat_deportes_nombre = '".$this->nombredeportes."' and id_eventonacional = ".$this->eventonacional);
	   $num_total_registros = parent::num_rows($consulta);
	   if($num_total_registros==0)
	    {		 
		  //se registrara un nuevo auto\		  
		  $consulta = parent::consulta("INSERT INTO cat_deportes (cat_deportes_nombre, id_eventonacional, id_usuario) VALUES ('".$this->nombredeportes."',".$this->eventonacional.",".$this->idusuario.")");	  
		 return (mysql_affected_rows() > 0)?'{"tipo":"succes","mensaje":"Operaci&oacute;n realizada con &eacute;xito..."}':'{"tipo":"error","mensaje":"No se grabo correctamente el Deporte..."}';	
	   }
	  else
	   {		   
		 //$mensaje['tipo']='error';$mensaje['mensaje']='N&uacute;mero de Serie del Auto Existente';  
		 return '{"tipo":"error","mensaje":"Deporte ya Existe, verifique..."}';
	   }	  
    }
	
	function grabartodoCatDeportes()
    {		
	    $array_ids = array();
		$insert_affected_deportes = '';
	    foreach (explode('<&>',$this->nombredeportes) as $id_act){			
		   $consulta = parent::consulta("SELECT id_deportes FROM cat_deportes where cat_deportes_nombre = '".$id_act."' And id_eventonacional = ".$this->eventonacional." And id_usuario = ".$this->idusuario);
		   $num_total_registros = parent::num_rows($consulta);		   					
		   if($num_total_registros==0)
	       {			   
 	         $consulta = parent::consulta("INSERT INTO cat_deportes (cat_deportes_nombre, id_eventonacional, id_usuario) VALUES ('".$id_act."',".$this->eventonacional.",".$this->idusuario.")");		   
			 $insert_affected_deportes .= (mysql_affected_rows() > 0)?'':' <'.$id_act.'> ';											  			
		   }else{$insert_affected_deportes .= ' <'.$id_act.'> ';}
		   
		}	
		return ($insert_affected_deportes == '')?'{"tipo":"succes","mensaje":"'.$insert_affected_deportes.'Operaci&oacute;n realizada con &eacute;xito..."}':'{"tipo":"error","mensaje":"'.$insert_affected_deportes.' Error al registrar, Verifique, Duplicidad..."}';		  
    }
	
	function aplicarCatDeportes()
	{	
	   $consulta = parent::consulta("SELECT cat_deportes_nombre FROM cat_deportes WHERE cat_deportes_nombre = '".$this->nombredeportes."' and id_eventonacional = ".$this->eventonacional);
	   $num_total_registros = parent::num_rows($consulta);
	   if($num_total_registros==0)
	    {
			 $consulta = parent::consulta("UPDATE cat_deportes SET cat_deportes_nombre = '".$this->nombredeportes."', id_eventonacional = ".$this->eventonacional.", id_usuario=".$this->idusuario." WHERE id_deportes = ".$this->id);		 
			 
		 return (mysql_affected_rows() > 0)?'{"tipo":"succes","mensaje":"Operaci&oacute;n realizada con &eacute;xito..."}':'{"tipo":"error","mensaje":"No se grabo correctamente el Deporte..."}';
		}else{
			
		}
	   return '{"tipo":"error","mensaje":"Nombre del deporte existente para Evento Nacional..."}';
	}	
	...
	function listar_deportes($catalogo,$ids,$idusuario)
	{		
	    $array_ids = array();
	...
	function mostrarCatDeportes()
	{
		...
		while($ban){
		  ...
		  if($this->filtro!='') $criterios_FILTRO = " r.cat_deportes_nombre LIKE \"%".$this->filtro."%\"";	 
		  ...
		  $consulta = parent::consulta("
			 ...
			 cat_deportes_nombre as nombredeportes,
			 cat_eventonacional_nombre as eventonacional,
			 id_deportes as iddeportes
			 FROM 
			 (cat_deportes as r inner join cat_eventonacional as en on r.id_eventonacional = en.id_eventonacional) inner join reg_usuario as u on r.id_usuario = u.id_usuario and r.id_usuario = ".$this->idusuario." ".$criterios_FILTRO.$criterios_ORDEN.$criterios_LIMITE);		  
		  ...
		  $consulta_totalregistro = parent::consulta("
			 ...
			 id_deportes as iddeportes
			 FROM 
			 (cat_deportes as r inner join cat_eventonacional as en on r.id_eventonacional = en.id_eventonacional) inner join reg_usuario as u on r.id_usuario = u.id_usuario and r.id_usuario = ".$this->idusuario." ".$criterios_FILTRO);
		...
		{		
                 ...
                 <thead><tr>
				    ...
				    <th width=\"15%\" class=\"title\">
                        <a href=\"javascript:tableOrdering('cat_deportes_nombre','".$fg->ordena('cat_deportes_nombre',$this->campo,$this->orden)."','');\" title=\"Haz click para ordenar por esta columna\">Nombre Deportes".$fg->ordenaimg('cat_deportes_nombre',$this->campo,$this->orden)."</a>
                    </th>					
					...
					<th width=\"6%\" class=\"title\"><a href=\"javascript:tableOrdering('id_deportes','".$fg->ordena('id_deportes',$this->campo,$this->orden)."','');\" title=\"Haz click para ordenar por esta columna\">Id".$fg->ordenaimg('id_deportes',$this->campo,$this->orden)."</a>
					</th>
				...
				$i=0;
				while($catdeportes = parent::fetch_array($consulta))
				{				
					extract($catdeportes);			
					...
					if($i%2==0){$flag = "class=\"row0\"";}else{$flag = "class=\"row1\"";}
					  $ii=$i+1;
					  print "
					  <tr id=\"$iddeportes\" $flag>
						<td style=\"text-align:center\">".$ii."</td>
						<td><input type=\"checkbox\" id=\"cb$i\" name=\"cid[]\" class=\"check_me\" value=\"$iddeportes\"/></td>				
						<td style=\"text-align:center\"><a href=\"../modulos/sitd_mod_guardaraplicar_catdeportes.php?id=$iddeportes&tipo=edit&texto=Editar Deportes\">$nombredeportes</a></td>
						<td style=\"text-align:center\">$eventonacional</td>
						<td style=\"text-align:center\">$nombre</td>																	
						<td style=\"text-align:center\">$iddeportes</td>
					  </tr>
					  ";
					  $i++;					
				}		
	...
	function cargareditarCatDeportes()
	{			 
	        $consulta = parent::consulta("SELECT 
			 ...
			 id_deportes as iddeportes
			 FROM 
			 cat_deportes as r inner join cat_eventonacional as en on r.id_eventonacional = en.id_eventonacional where id_deportes = ".$this->id);
