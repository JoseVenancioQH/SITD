...
class desplegarvalidados extends MySQL
{					
    var $municipio = '';
	var $evento = '';	
	var $modalidad = '';
	...
	function GeneraValidados()
	  {	   	
			...
			$consulta = parent::consulta("SET @@group_concat_max_len = 9999999;");
			$consulta = parent::consulta(""
			."select "			
			.$filtroconvivencia
			."reg_p.id_registro, "			  	         			 
			."reg_participante_documentos as documentos, "
			...
			."group_concat(CONCAT('<table width=\"100%\"><tr><td width=\"80%\">',concat_WS('<br />', cat_m.cat_modalidad_nombre, cat_d.cat_deporte_nombre, cat_c.cat_categoria_nombre, reg_c.reg_categoriaparticipante_pruebas, reg_m.reg_modalidadparticipante_cargo),'</td>','<td class=\"validap\" id=\"td',reg_c.id_categoriapar,'\" width=\"20%\"><img id=\"',reg_c.id_categoriapar,'\" style=\"cursor:pointer;\" onclick=\"javascript:validar(\'',reg_c.reg_categoriaparticipante_validado,'\',\'',reg_p.id_registro,'\',\'',reg_c.id_categoriapar,'\')\" src=\"../img/',reg_c.reg_categoriaparticipante_validado,'.png\" />','</td></tr></table>') SEPARATOR '<br />') AS caracteristicas, " 
