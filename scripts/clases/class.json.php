...
{	
	...
	function GenerarJson()
	  ...
	  $filtro = $filtromunicipio.$filtroano.$filtromodalidad.$filtrodeporte.$filtrorama.$filtronombres.$filtroappaterno.$filtroapmaterno;
	  ...
	  if(!empty($filtro)){
	   ...
	   /*$consulta = parent::consulta("select			
			  ...
			  from
			  (((((reg_participante as reg_p inner join reg_modalidadparticipante as reg_m on reg_p.id_registro = reg_m.id_registro) inner join reg_categoriaparticipante as reg_c on reg_c.id_registro = reg_p.id_registro) inner join cat_municipio as cat_mu on cat_mu.id_municipio = reg_p.id_municipio) inner join cat_categoria as cat_c on reg_c.id_categoria = cat_c.id_categoria) inner join cat_deportes as cat_d on cat_c.id_deporte = cat_d.id_deporte) inner join cat_modalidad as cat_m on reg_m.id_modalidad = cat_m.id_modalidad $filtro GROUP BY reg_p.id_registro");*/
	   ...
	   /*$consulta = parent::consulta("select "			
			 ...
			 ."cat_mu.cat_municipio_nombre as municipio "
			 ."from "
		     ."(((reg_participante as reg_p inner join cat_municipio as cat_mu on reg_p.id_municipio = cat_mu.id_municipio) left join ((reg_modalidadparticipante as reg_m inner join cat_modalidad as cat_m on reg_m.id_modalidad = cat_m.id_modalidad) left join  cat_deportes as cat_dd on reg_m.reg_modalidadparticipante_deporte = cat_dd.id_deporte) on reg_p.id_registro = reg_m.id_registro) inner join (reg_eventoparticipante as reg_ep inner join reg_eventos as reg_e on reg_ep.id_evento = reg_e.id_evento) on reg_p.id_registro = reg_ep.id_registro) left join ((reg_categoriaparticipante as reg_c inner join cat_categoria as cat_c on reg_c.id_categoria = cat_c.id_categoria) inner join cat_deportes as cat_d on cat_c.id_deporte = cat_d.id_deporte  ) on reg_p.id_registro = reg_c.id_registro $filtro GROUP BY reg_p.id_registro ORDER BY $this->sidx $this->sord LIMIT $start , $this->limit");	*/
		...
		{					  
			   ...
			   {		
		        ...
		        $responce->rows[$i]['cell']=array(
											      ...
											      (file_exists("../fotosparticipantes/".$actual["curp"].".png")) ? "<img id='foto".$actual["curp"]."' src='../fotosparticipantes/".$actual["curp"].".png' onmouseover='javascript:actualizarfoto(\"".$actual["curp"]."\");' onclick='javascript:actualizarfoto(\"".$actual["curp"]."\");'/>" : "<img id='foto".$actual["curp"]."'  src='../img/foto_renovar.jpg' onmouseover='javascript:actualizarfoto(\"".$actual["curp"]."\");' onclick='javascript:actualizarfoto(\"".$actual["curp"]."\");'/>");
		        $i++;              
			   }
