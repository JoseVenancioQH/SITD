...
class cargardatosparticipante extends MySQL
{	
    var $idregistro = '';
	var $evento = '';
	
	function CargarDatosParticipantes_Editar()
	  {	   	          
			 $consulta = parent::consulta("SELECT
			 reg_p.id_registro as idregistro,							  
			 ...
			 FROM reg_participante as reg_p where reg_p.id_registro = '".$this->idregistro."'");		 
			 $num_total_registros = parent::num_rows($consulta);
			 if($num_total_registros>0)
			 {
			   while ($actual = parent::fetch_assoc($consulta)) 
			   {			    		    
			     $actual['fechanac'] = date('d-m-Y', strtotime ($actual['fechanac']));
				 $actual['foto'] = (file_exists("../fotosparticipantes/".$actual["curp"].".png")) ? "<img id='foto".$actual["curp"]."' src='../fotosparticipantes/".$actual["curp"].".png' onmouseover='javascript:actualizarfoto(\"".$actual["curp"]."\");' onclick='javascript:actualizarfoto(\"".$actual["curp"]."\");'/>" : "<img id='foto".$actual["curp"]."'  src='../img/foto.png' onmouseover='javascript:actualizarfoto(\"".$actual["curp"]."\");' onclick='javascript:actualizarfoto(\"".$actual["curp"]."\");'/>";
				/* $actual['modalidadcategoria'] = [];*/
				
				 $consulta2 = parent::consulta("select			
				 cat_m.cat_modalidad_nombre as modalidad,
				 cat_m.id_modalidad as idmodalidad,
				 cat_d.cat_deporte_nombre as deporte,
				 cat_d.id_deporte as iddeporte,
				 reg_ee.reg_eventos_nombre as evento,
				 reg_ee.id_evento as idevento,
				 cat_c.cat_categoria_nombre as categoria,
				 cat_c.id_categoria as idcategoria,
				 ...
				 inner join reg_eventoparticipante as reg_e on reg_p.id_registro = reg_e.id_registro and reg_p.id_registro = $this->idregistro)
				 inner join reg_modalidadparticipante as reg_m on reg_e.id_regevento = reg_m.id_regevento and reg_e.id_evento = $this->evento)
				 ...
				 $num_total_registros2 = parent::num_rows($consulta2);			   
				 if($num_total_registros2>0)
				 {
				   while ($actual_modalidadcategoria = parent::fetch_assoc($consulta2)) 
				   {	    
					$arrData_modalidadcategoria[] = $actual_modalidadcategoria; 
				   }
				 }			 
				 $actual['modalidadcategoria'] = $arrData_modalidadcategoria;				 
				 $arrData[] = $actual;			                 
			   }			   
			   return $arrData;		   
			 }
