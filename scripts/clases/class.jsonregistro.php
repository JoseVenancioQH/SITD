...
class jsonregistro extends MySQL
{	
	...
	function GenerarJson()
		...
		{					  
			   ...
			   {		
		        ...
		        $responce->rows[$i]['cell']=array(
						"<img id='eliminar".$actual['id_registro']."' style='vertical-align:middle; cursor:pointer;' src='../img/icons/delete.png' onclick='javascript:eliminarparticipante(\"".$actual['id_registro']."\");'/>",
						"<img id='actualizar".$actual['id_registro']."' style='vertical-align:middle; cursor:pointer;' src='../img/icons/edit.png' onclick='javascript:editarparticipante(\"".$actual['id_registro']."\");'/>",
						$actual['curp'],
						$actual['nombre'],
						$actual['appaterno'],
						$actual['apmaterno'],
						...
						/*(file_exists("../fotosparticipantes/".$actual["curp"].".png")) ? "<div class = 'editimg'><img id='foto".$actual["curp"]."' src='../fotosparticipantes/".$actual["curp"].".png' onmouseover='javascript:actualizarfoto(\"".$actual["curp"]."\");' onclick='javascript:actualizarfoto(\"".$actual["curp"]."\");'/></div>" : "<img id='foto".$actual["curp"]."'  src='../img/foto_renovar.jpg' onmouseover='javascript:actualizarfoto(\"".$actual["curp"]."\");' onclick='javascript:actualizarfoto(\"".$actual["curp"]."\");'/>");*/
				       /*(file_exists("../fotosparticipantes/".$actual["curp"].".png")) ? "<img class = 'editimg' id='".$actual["curp"]."' src='../fotosparticipantes/".$actual["curp"].".png' onmouseover='javascript:actualizarfoto(\"".$actual["curp"]."\");'/>" : "<img id='foto".$actual["curp"]."'  src='../img/foto_renovar.jpg' onmouseover='javascript:actualizarfoto(\"".$actual["curp"]."\");' onclick='javascript:actualizarfoto(\"".$actual["curp"]."\");'/>");*/
		        $i++;              
			   }
	  ...
	  function GenerarJsonUltimosRegistrados()
	  {	     
		if(empty($this->evento)) $filtroevento = ''; else $filtroevento = " and reg_e.id_evento = ".$this->evento;
		
		if(empty($this->municipio) || $this->municipio == 0) $filtromunicipio = ''; else $filtromunicipio = " and cat_mu.id_municipio = ".$this->municipio;
		
		$filtro = $filtroevento.$filtromunicipio;	  
		if(!empty($filtro)){
			 $filtro = substr($filtro,5,strlen($filtro));		
			 $filtro = ' WHERE '.$filtro;
		}   
		...
		{					  
			   ...
			   {		
		        ...
		        $responce->rows[$i]['cell']=array(
						...
						date('d-m-Y', strtotime ($actual['fechanac'])),
						$actual['sexo'],	
						/*(file_exists("../fotosparticipantes/".$actual["curp"].".png")) ? "<div class='editimg' ><img id='".$actual["curp"]."' src='../fotosparticipantes/".$actual["curp"].".png'/></div>" : "<img id='foto".$actual["curp"]."'  src='../img/foto_renovar.jpg' onmouseover='javascript:actualizarfoto(\"".$actual["curp"]."\");' onclick='javascript:actualizarfoto(\"".$actual["curp"]."\");'/>");*/				         
						(file_exists("../fotosparticipantes/".$actual["curp"].".png")) ? "<img class='edit_foto' style=' vertical-align:top; cursor:pointer;' src='../img/editar_img.jpg' onclick='javascript:editarfoto(\"".$actual["curp"]."\",\"\");'/><br /><img id='foto".$actual["curp"]."' src='../fotosparticipantes/".$actual["curp"].".png?nocache=".(rand()*1000)."' style='vertical-align:top; cursor:pointer;' onmouseover='javascript:actualizarfoto(\"".$actual["curp"]."\");' onclick='javascript:actualizarfoto(\"".$actual["curp"]."\");'/>" : "<img id='foto".$actual["curp"]."'  src='../img/foto_renovar.jpg' style='vertical-align:middle; cursor:pointer;' onmouseover='javascript:actualizarfoto(\"".$actual["curp"]."\");' onclick='javascript:actualizarfoto(\"".$actual["curp"]."\");'/>");
		        $i++;              
			   }
