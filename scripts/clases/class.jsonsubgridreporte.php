...
class jsonsubgridreporte extends MySQL
{	
    var $validado = 0;
	var $id = 0;
	var $evento = 0;// get index row - i.e. user click to sort	
	function GenerarJsonSubgridReporte()
	  {	  	      
	  ...
	  if(empty($this->validado)) $filtrovalidado = ''; else $filtrovalidado = ' where reg_c.reg_categoriaparticipante_validado = "'.$this->validado.'"';
	  ...
	  reg_c.reg_categoriaparticipante_pruebas as pruebas,
	  reg_c.id_categoriapar as id_categoriapar,
	  reg_c.reg_categoriaparticipante_validado as validado_des
	  from
	  ...
	  left join reg_eventos as reg_ee on reg_e.id_evento = reg_ee.id_evento $filtro");	  
