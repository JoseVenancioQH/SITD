...
class grabarcomiteorganizador extends MySQL
{  	
    var $prensa = '';
    var $cargo = '';
	var $deporteextra = '';
...
function grabarParticipanteComiteOrganizador()
	{	
...
////Registro Modalidad		
	   ...
	   if($num_total_registros==0)
		 {
		  if(empty($this->cargo)) $this->cargo = $this->prensa; 
		  //se registrara una nueva modalidad para el participante	  
		  $consulta = parent::consulta("INSERT INTO reg_modalidadparticipante (id_evento,id_registro,id_modalidad,reg_modalidadparticipante_cargo) VALUES (".$this->evento.",".$id_insert_participante.",".$this->modalidad.",'".$this->cargo."')");		  
