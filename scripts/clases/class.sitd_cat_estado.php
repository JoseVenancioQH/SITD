...
{   
	...
	function borrarCatEstado()
	{		    
	    $array_ids = array();	
		
		foreach (explode(',',$this->ids) as $id_act){	
		         $consulta = parent::consulta("SELECT id_estado FROM cat_estado as e WHERE e.id_estado = ".$id_act);
				 $num_total_registros = parent::num_rows($consulta);
				 if($num_total_registros!=0)
				  {	
					$consulta = parent::consulta("DELETE FROM cat_estado where id_estado = ".$id_act);
					if(mysql_affected_rows() > 0) $array_ids[]=$id_act;
				  }				
		}
	    return implode(',',$array_ids);
	...
	function grabarCatEstado()
    {
	   $consulta = parent::consulta("SELECT cat_estado_nombre FROM cat_estado WHERE cat_estado_nombre = '".$this->nombreestado."'");
	   $num_total_registros = parent::num_rows($consulta);
	   if($num_total_registros==0)
	    {		 
		  //se registrara un nuevo auto\		  
		  $consulta = parent::consulta("INSERT INTO cat_estado (cat_estado_nombre, id_usuario) VALUES ('".$this->nombreestado."',".$this->idusuario.")");	  
		 return (mysql_affected_rows() > 0)?'{"tipo":"succes","mensaje":"Operaci&oacute;n realizada con &eacute;xito..."}':'{"tipo":"error","mensaje":"No se grabo correctamente el estado..."}';	
	   }
	  else
	   {		   
	...
	function aplicarCatEstado()
	{	
	   $consulta = parent::consulta("SELECT cat_estado_nombre FROM cat_estado WHERE cat_estado_nombre = '".$this->nombreestado."'");
	   $num_total_registros = parent::num_rows($consulta);
	   if($num_total_registros==0)
	    {
			 $consulta = parent::consulta("UPDATE cat_estado SET cat_estado_nombre = '".$this->nombreestado."', id_usuario=".$this->idusuario." WHERE id_estado = ".$this->id);		 
			 
		 return (mysql_affected_rows() > 0)?'{"tipo":"succes","mensaje":"Operaci&oacute;n realizada con &eacute;xito..."}':'{"tipo":"error","mensaje":"No se grabo correctamente el estado..."}';
		}else{
			
		}
	   return '{"tipo":"error","mensaje":"Nombre de estado existente..."}';
	}	
	...
	function mostrarCatEstado()
	{
		...
		while($ban){
		  ...
		  if($this->filtro!='') $criterios_FILTRO = " e.cat_estado_nombre LIKE \"%".$this->filtro."%\"";	 
		  ...
		  $consulta = parent::consulta("
			 ...
			 FROM 
			 cat_estado as e inner join reg_usuario as u on e.id_usuario = u.id_usuario and e.id_usuario = ".$this->idusuario." ".$criterios_FILTRO.$criterios_ORDEN.$criterios_LIMITE);
		  ...
		  $consulta_totalregistro = parent::consulta("
			 ...
			 id_estado as idestado
			 FROM 
			 cat_estado as e inner join reg_usuario as u on e.id_usuario = u.id_usuario and e.id_usuario = ".$this->idusuario." ".$criterios_FILTRO);
		...
		{		
                 ...
                 <thead><tr>
					...
					<th width=\"6%\" class=\"title\"><a href=\"javascript:tableOrdering('id_estado','".$fg->ordena('id_estado',$this->campo,$this->orden)."','');\" title=\"Haz click para ordenar por esta columna\">Id".$fg->ordenaimg('id_estado',$this->campo,$this->orden)."</a>
					</th>
				...
				{				
					...
					if($i%2==0){$flag = "class=\"row0\"";}else{$flag = "class=\"row1\"";}
					  ...
					  <tr id=\"$idestado\" $flag>
						...
						<td><input type=\"checkbox\" id=\"cb$i\" name=\"cid[]\" class=\"check_me\" value=\"$idestado\"/></td>				
						<td style=\"text-align:center\"><a href=\"../modulos/sitd_mod_guardaraplicar_catestado.php?id=$idestado&tipo=edit&texto=Editar Estado\">$nombreestado</a></td>
	...
	function cargareditarCatEstado()
	{			 
	        $consulta = parent::consulta("SELECT 
			 ...
			 id_estado as idestado
			 FROM 
			 cat_estado as e where id_estado = ".$this->id);
