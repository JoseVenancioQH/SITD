...
{   
	...
	function borrarCatModDep()
	{		    
	    $array_ids = array();	
		
		foreach (explode(',',$this->ids) as $id_act){	
		         $consulta = parent::consulta("SELECT id_moddep FROM cat_moddep as r WHERE r.id_moddep = ".$id_act);
				 $num_total_registros = parent::num_rows($consulta);
				 if($num_total_registros!=0)
				  {	
					$consulta = parent::consulta("DELETE FROM cat_moddep where id_moddep = ".$id_act);
					if(mysql_affected_rows() > 0) $array_ids[]=$id_act;
				  }				
		}
	    return implode(',',$array_ids);
	...
	function grabarCatModDep()
    {
	   $consulta = parent::consulta("SELECT cat_moddep_nombre FROM cat_moddep WHERE cat_moddep_nombre = '".$this->nombremoddep."' and id_eventonacional = ".$this->eventonacional);
	   $num_total_registros = parent::num_rows($consulta);
	   if($num_total_registros==0)
	    {		 
		  //se registrara un nuevo auto\		  
		  $consulta = parent::consulta("INSERT INTO cat_moddep (cat_moddep_nombre, id_eventonacional, id_usuario) VALUES ('".$this->nombremoddep."',".$this->eventonacional.",".$this->idusuario.")");	  
		 return (mysql_affected_rows() > 0)?'{"tipo":"succes","mensaje":"Operaci&oacute;n realizada con &eacute;xito..."}':'{"tipo":"error","mensaje":"No se grabo correctamente la modalidad deportiva..."}';	
	   }
	  else
	   {		   
		 //$mensaje['tipo']='error';$mensaje['mensaje']='N&uacute;mero de Serie del Auto Existente';  
		 return '{"tipo":"error","mensaje":"Modalidad Deportiva ya Existe, verifique..."}';
	   }	  
    }
	
	function grabartodoCatModDep()
    {		
	    $array_ids = array();
		$insert_affected_moddep = '';
	    foreach (explode('<&>',$this->nombremoddep) as $id_act){			
		   $consulta = parent::consulta("SELECT id_moddep FROM cat_moddep where cat_moddep_nombre = '".$id_act."' And id_eventonacional = ".$this->eventonacional." And id_usuario = ".$this->idusuario);
		   $num_total_registros = parent::num_rows($consulta);		   					
		   if($num_total_registros==0)
	       {			   
 	         $consulta = parent::consulta("INSERT INTO cat_moddep (cat_moddep_nombre, id_eventonacional, id_usuario) VALUES ('".$id_act."',".$this->eventonacional.",".$this->idusuario.")");		   
			 $insert_affected_moddep .= (mysql_affected_rows() > 0)?'':' <'.$id_act.'> ';											  			
		   }else{$insert_affected_moddep .= ' <'.$id_act.'> ';}
		   
		}	
		return ($insert_affected_moddep == '')?'{"tipo":"succes","mensaje":"'.$insert_affected_moddep.'Operaci&oacute;n realizada con &eacute;xito..."}':'{"tipo":"error","mensaje":"'.$insert_affected_moddep.' Error al registrar, Verifique, Duplicidad..."}';		  
    }
	
	function aplicarCatModDep()
	{	
	   $consulta = parent::consulta("SELECT cat_moddep_nombre FROM cat_moddep WHERE cat_moddep_nombre = '".$this->nombremoddep."' and id_eventonacional = ".$this->eventonacional);
	   $num_total_registros = parent::num_rows($consulta);
	   if($num_total_registros==0)
	    {
			 $consulta = parent::consulta("UPDATE cat_moddep SET cat_moddep_nombre = '".$this->nombremoddep."', id_eventonacional = ".$this->eventonacional.", id_usuario=".$this->idusuario." WHERE id_moddep = ".$this->id);		 
			 
		 return (mysql_affected_rows() > 0)?'{"tipo":"succes","mensaje":"Operaci&oacute;n realizada con &eacute;xito..."}':'{"tipo":"error","mensaje":"No se grabo correctamente la modalidad deportiva..."}';
		}else{
			
		}
	   return '{"tipo":"error","mensaje":"Nombre de modalidad deportiva existente para Evento Nacional..."}';
	}	
	...
	function listar_moddep($catalogo,$ids,$idusuario)
	{		
	    $array_ids = array();
	...
	function mostrarCatModDep()
	{
		...
		while($ban){
		  ...
		  if($this->filtro!='') $criterios_FILTRO = " r.cat_moddep_nombre LIKE \"%".$this->filtro."%\"";	 
		  ...
		  $consulta = parent::consulta("
			 ...
			 cat_moddep_nombre as nombremoddep,
			 cat_eventonacional_nombre as eventonacional,
			 id_moddep as idmoddep
			 FROM 
			 (cat_moddep as r inner join cat_eventonacional as en on r.id_eventonacional = en.id_eventonacional) inner join reg_usuario as u on r.id_usuario = u.id_usuario and r.id_usuario = ".$this->idusuario." ".$criterios_FILTRO.$criterios_ORDEN.$criterios_LIMITE);
		  ...
		  $consulta_totalregistro = parent::consulta("
			 ...
			 id_moddep as idmoddep
			 FROM 
			 (cat_moddep as r inner join cat_eventonacional as en on r.id_eventonacional = en.id_eventonacional) inner join reg_usuario as u on r.id_usuario = u.id_usuario and r.id_usuario = ".$this->idusuario." ".$criterios_FILTRO);
		...
		{		
                 ...
                 <thead><tr>
				    ...
				    <th width=\"15%\" class=\"title\">
                        <a href=\"javascript:tableOrdering('cat_moddep_nombre','".$fg->ordena('cat_moddep_nombre',$this->campo,$this->orden)."','');\" title=\"Haz click para ordenar por esta columna\">Nombre Modalidad Deportiva".$fg->ordenaimg('cat_moddep_nombre',$this->campo,$this->orden)."</a>
                    </th>					
					...
					<th width=\"6%\" class=\"title\"><a href=\"javascript:tableOrdering('id_moddep','".$fg->ordena('id_moddep',$this->campo,$this->orden)."','');\" title=\"Haz click para ordenar por esta columna\">Id".$fg->ordenaimg('id_moddep',$this->campo,$this->orden)."</a>
					</th>
				...
				{				
					...
					if($i%2==0){$flag = "class=\"row0\"";}else{$flag = "class=\"row1\"";}
					  ...
					  <tr id=\"$idmoddep\" $flag>
						...
						<td><input type=\"checkbox\" id=\"cb$i\" name=\"cid[]\" class=\"check_me\" value=\"$idmoddep\"/></td>				
						<td style=\"text-align:center\"><a href=\"../modulos/sitd_mod_guardaraplicar_catmoddep.php?id=$idmoddep&tipo=edit&texto=Editar Modalidad Deportiva\">$nombremoddep</a></td>
						<td style=\"text-align:center\">$eventonacional</td>
						<td style=\"text-align:center\">$nombre</td>																	
						<td style=\"text-align:center\">$idmoddep</td>
					  </tr>
					  ";
					  $i++;					
				}		
	...
	function cargareditarCatModDep()
	{			 
	        $consulta = parent::consulta("SELECT 
			 ...
			 id_moddep as idmoddep
			 FROM 
			 cat_moddep as r inner join cat_eventonacional as en on r.id_eventonacional = en.id_eventonacional where id_moddep = ".$this->id);
